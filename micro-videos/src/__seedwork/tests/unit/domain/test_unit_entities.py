...
import unittest
import uuid
from __seedwork.domain.entities import AggregateRoot, Entity
...
class TestEntityUnit(unittest.TestCase):
    ...
    def test_set_id_and_props(self):
        ...
        self.assertIsInstance(entity.unique_entity_id.id, uuid.UUID)
...
class TestAggregateRootUnit(unittest.TestCase):
    ...
    def test_if_is_dataclass(self):
        self.assertTrue(is_dataclass(AggregateRoot))

    def test_if_is_a_abstract_class(self):
        self.assertIsInstance(AggregateRoot(), ABC)
